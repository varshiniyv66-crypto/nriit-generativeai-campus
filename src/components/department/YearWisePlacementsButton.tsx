...
import { Search, Download, Calendar, TrendingUp, Briefcase } from "lucide-react";
...
interface YearWisePlacementsButtonProps {
    department: string;
}

const AVAILABLE_YEARS = ["2024-25", "2023-24", "2022-23"];

export function YearWisePlacementsButton({ department }: YearWisePlacementsButtonProps) {
            ...
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                <DialogContent className="max-w-[95vw] w-[1200px] h-[90vh] flex flex-col p-0 overflow-hidden rounded-3xl border-[#D4AF37]/30 bg-white/95 backdrop-blur-2xl">
                    ...
                    <div className="flex-1 overflow-auto p-8 pt-4">
                        <div className="rounded-2xl border border-slate-200 overflow-hidden overflow-x-auto shadow-sm bg-white">
                            <Table>
                                <TableHeader>
                    ...
                    <div className="p-6 border-t border-slate-100 bg-white flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2 text-xs font-bold text-blue-600 bg-blue-50 px-3 py-1 rounded-full border border-blue-100 uppercase tracking-tighter">
                                <TrendingUp className="w-3 h-3" />
                                Growth Verified
                            </div>
                        </div>
                        <div className="text-[10px] text-slate-400 font-medium uppercase tracking-[0.2em]">
                            NRIT Training & Placement Cell - Career Excellence
                        </div>
                    </div>
                </DialogContent>
            </Dialog>
